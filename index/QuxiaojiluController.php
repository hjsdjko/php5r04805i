...
session_start();
class QuxiaojiluController extends CommonController {
    ...
    public function page(){
        ...
        $sql = "select * from `quxiaojilu` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $lists = "select * from `quxiaojilu` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function list(){
        ...
        $page_count = ceil($numberCount/$limt);//页数
        $startCount = ($page-1)*$limt;
        $lists = "select * from `quxiaojilu` ".$where." order by ".$sortorders." limit ".$startCount.",".$limt;
        $result = table_sql($lists);
        $arrayData = array();
        if ($result->num_rows > 0) {
            while ($datas = $result->fetch_assoc()){
    ...
    public function save(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
        $sql = "INSERT INTO `quxiaojilu` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
    ...
    public function update(){
        ...
        $value = implode(',',$v);
        $sql = "UPDATE quxiaojilu SET ".$value." where id = ".$id;
        $result = table_sql($sql);
    ...
    public function delete(){
        ...
        $sql = "delete from quxiaojilu WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    ...
    public function query(){
        ...
        $sql = "select * from `quxiaojilu` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function value(){
        ...
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM quxiaojilu ".$where." group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM quxiaojilu ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM quxiaojilu ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM quxiaojilu ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function valueMul(){
        ...
        foreach(explode(",", $yColumnName) as $item){
            $sql = "SELECT ".$xColumnName.",sum(".$item.") total FROM quxiaojilu ".$where." group by ".$xColumnName." LIMIT 10";
            if(urldecode($request[6]) == '日') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$item.") total FROM quxiaojilu ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') LIMIT 10";
            }
            if(urldecode($request[6]) == '月') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$item.") total FROM quxiaojilu ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m') LIMIT 10";
            }
            if(urldecode($request[6]) == '年') {
                $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$item.") total FROM quxiaojilu ".$where."  GROUP BY DATE_FORMAT(".$xColumnName.", '%Y') LIMIT 10";
            }
            $result = table_sql($sql);
    ...
    public function group($columnName){
        ...
        $sql = "SELECT ".$columnName.",count(".$columnName.") as total FROM quxiaojilu ".$where." GROUP BY ".$columnName." ORDER BY $columnName asc";
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            $total = array();
    ...
    public function shBatch(){
        ...
        preg_match_all('/\d+/',$ids,$arr);
        $str = implode(',',$arr[0]);//拼接字符，
        $sql = "update quxiaojilu set sfsh = '".$_GET['sfsh']."', shhf='".$_GET['shhf']."' WHERE id in(".$str.")";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
