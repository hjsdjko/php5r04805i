...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}'  label="资源名称" prop="ziyuanmingcheng">
            <el-select v-model="ruleForm.ziyuanmingcheng" placeholder="请选择资源名称" :disabled=" false  ||ro.ziyuanmingcheng" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="使用时间" prop="shiyongshijian">
              <el-date-picker
				  :disabled=" false  ||ro.shiyongshijian"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="自身位置" prop="zishenweizhi">
            <el-input v-model="ruleForm.zishenweizhi" 
                placeholder="自身位置" clearable :disabled=" false  ||ro.zishenweizhi"></el-input>
          </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/ziyuanxinxi/ziyuanmingcheng', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yuyueziyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yuyueziyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yuyueziyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
