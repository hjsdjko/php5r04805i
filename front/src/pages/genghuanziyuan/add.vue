...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="使用时间" prop="shiyongshijian">
            <el-input v-model="ruleForm.shiyongshijian" 
                placeholder="使用时间" clearable :disabled=" false  ||ro.shiyongshijian"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="申请补发时间" prop="shenqingbufashijian">
              <el-date-picker
				  :disabled=" false  ||ro.shenqingbufashijian"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="补发原因" prop="bufayuanyin">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="补发原因"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`genghuanziyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('genghuanziyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`genghuanziyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
