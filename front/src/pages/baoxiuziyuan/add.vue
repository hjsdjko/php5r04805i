...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}'  label="维修状态" prop="weixiuzhuangtai">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="故障情况" prop="guzhangqingkuang">
            <el-input
              ...
              :rows="8"
              placeholder="故障情况"
              v-model="ruleForm.guzhangqingkuang">
            </el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`baoxiuziyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('baoxiuziyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`baoxiuziyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
