<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="资源名称" prop="ziyuanmingcheng">
					<el-select :disabled="ro.ziyuanmingcheng" v-model="ruleForm.ziyuanmingcheng" placeholder="请选择资源名称" >
						<el-option
							v-for="(item,index) in ziyuanmingchengOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="自身位置" prop="zishenweizhi">
					<el-input v-model="ruleForm.zishenweizhi" placeholder="自身位置" clearable  :readonly="ro.zishenweizhi"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="自身位置" prop="zishenweizhi">
					<el-input v-model="ruleForm.zishenweizhi" placeholder="自身位置" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="用途" prop="yongtu">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.yongtu" label="用途" prop="yongtu">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.yongtu}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='zishenweizhi'){
							this.ruleForm.zishenweizhi = obj[o];
							this.ro.zishenweizhi = true;
							continue;
						}
				}
			}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `yuyueziyuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuyueziyuanCrossAddOrUpdateFlag = false;
											this.parent.search();
