<template>
	<div class="main-content" :style='{"padding":"30px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="资源名称" prop="ziyuanmingcheng">
						...
						<el-select clearable v-model="searchForm.ziyuanmingcheng" placeholder="请选择资源名称" >
							<el-option v-for="(item,index) in ziyuanmingchengOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuyueziyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yuyueziyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu9" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuyueziyuan','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuyueziyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueziyuan','报修')" @click="baoxiuziyuanCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								报修
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueziyuan','归还')" @click="guihaiziyuanCrossAddOrUpdateHandler(scope.row,'cross','是','','','')" type="success">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								归还
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuyueziyuan','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yuyueziyuan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont " :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<baoxiuziyuan-cross-add-or-update v-if="baoxiuziyuanCrossAddOrUpdateFlag" :parent="this" ref="baoxiuziyuanCrossaddOrUpdate"></baoxiuziyuan-cross-add-or-update>
...
import baoxiuziyuanCrossAddOrUpdate from "../baoxiuziyuan/add-or-update";
import guihaiziyuanCrossAddOrUpdate from "../guihaiziyuan/add-or-update";
	export default {
		data() {
			return {
				ziyuanmingchengOptions: [],
				...
				baoxiuziyuanCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			baoxiuziyuanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.baoxiuziyuanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyueziyuan');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.baoxiuziyuanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.baoxiuziyuanCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    init () {
        this.sfshOptions = "是,否,待审核".split(',');
          this.$http({
            url: `option/ziyuanxinxi/ziyuanmingcheng`,
            method: "get"
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.ziyuanmingchengOptions = data.data;
            } else {
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "yuyueziyuan/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
	...
	yuyueziyuanstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'yuyueziyuan/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
